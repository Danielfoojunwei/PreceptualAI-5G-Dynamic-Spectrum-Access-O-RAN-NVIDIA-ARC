...
|---|---|---|---|
| **GV.OC-04** Critical objectives, capabilities, and services that stakeholders depend on are understood and communicated | The deployer must know what Horizon-RIC's purpose is and what it depends on | `docs/RESEARCH_ALIGNMENT.md` (purpose, scope); `docs/THREAT_MODEL.md` (residual-risk register); `README.md` | None |
| **GV.RM-01** Risk management objectives are established and agreed to by organisational stakeholders | Documented risk objectives | `docs/THREAT_MODEL.md`; `docs/EVALUATION_CRITERIA.md` (standards/regulation gap analysis); `docs/conformance/CONFORMANCE.md` | None at the rApp tier; operator must complete its own risk register |
...
| **ID.RA-01** Vulnerabilities in assets are identified, validated, and recorded | Vulnerability inventory | CI dependency-scan via SBOM (`deploy/sbom/horizon-ric-sbom.json`, validated by `deploy/sbom/validate_sbom.py`); `docs/THREAT_MODEL.md` (residual-risk register) | NOT YET — automated CVE feed integration planned in Phase 3 |
...
| **PR.PS-01** Configuration management practices are established and applied | Configuration management | `deploy/helm/`, `deploy/docker/`, `deploy/systemd/`, `deploy/docker-compose.yml`; `pyproject.toml`; environment-variable surface in `src/horizon_ric/rapp/lifecycle.py` (`_config_from_env`) | None |
...
| Sub-category | What it requires | Horizon-RIC mapping | Gap |
|---|---|---|---|
| **DE.CM-01** Networks and network services are monitored to find potentially adverse events | Continuous network monitoring | Prometheus metrics at `/metrics`: `src/horizon_ric/rapp/health.py`; SLA breach detection via Prometheus alert rules at `deploy/prometheus/sla_rules.yml` (e.g. `HorizonSLALatencyP99`, `HorizonSLAA1EmitSuccess`), `deploy/prometheus/rules.yml` | None |
...
| **RS.AN-03** Analysis is performed to establish what has taken place during an incident and the root cause | Forensic analysis | DecisionRecord hash chain (`evidence/store.py`); each record carries a `decision_id` and `sla_breach_context` correlating breaches to the decision in flight (`evidence/schema.py`); per-tenant chain verification (`evidence/store.py`, `verify_tenant`) | None |
...
| **RS.CO-02** Internal stakeholders are notified of incidents | Operator notification | NIS-2 incident webhook reporter: `src/horizon_ric/security/nis2_reporter.py` (`NIS2IncidentReporter`); ITU-T X.733 alarm records for SMO/NMS fault ingestion: `src/horizon_ric/observability/x733_alarms.py`; Prometheus alert rules at `deploy/prometheus/sla_rules.yml` | None |
