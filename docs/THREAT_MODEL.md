...
|-----|----------------|--------|
| **No differential-privacy accountant** | ML03 model inversion, ML04 membership inference | **MECHANISM-LEVEL ADDRESSED** (§9) — DP-FedAvg reports a one-round bound with public clipping and conservative replace-one sensitivity. The 64/64, 10-seed MIA result is an attack diagnostic, not privacy proof; lifecycle composition remains open. |
...
|---|---|---|---|
| **DP-FedAvg + Rényi-DP accountant** (`federated/dp.py`, `dp_privacy.json`) | Liu, Jiang, Lam et al., *Efficient FU with Adaptive DP*, IEEE BigData 2024; accounting per Mironov CSF-17 / Abadi CCS-16 | Public L2 clipping at C=12 + Gaussian noise calibrated to conservative replace-one sensitivity 2C give a one-round (ε, δ) bound. The attack diagnostic now uses 64 members, 64 non-members and 10 independent seeds: at z=8, **mean MIA AUC 0.5207, 95% CI [0.4696, 0.5647]** | AUC near chance is not privacy proof. The bound is for the stated one-round mechanism, not unaccounted lifecycle steps. Utility falls from throughput 1.2318 (no DP) to 0.1885 (z=8). |
| **Verifiable two-server secure aggregation** (`federated/verifiable_secagg.py`, `verifiable_secagg.json`) | Liu, Ye, Jiang, Shen, Guo, Tjuawinata & Lam, Starfish, arXiv:2404.09724 | 2-of-2 additive secret sharing hides a raw contribution from either server alone; Feldman commitments detect tampered or dropped contributions (1.0 in 200-trial probes). Opt-in client-side local DP ensures colluding servers reconstruct a clipped/noisy release instead of raw data. | Exact-mode privacy holds **only under non-collusion**. Local-DP protection depends on public clip/noise/accounting assumptions and has large measured utility distortion; Feldman commitments are binding, not hiding. |
