...
**Companion docs:** `GAPS_TO_PILOT.md` §D-2 (the original concession), `MARKETPLACE_POSITIONING.md` ("FIPS readiness" appendix), `docs/compliance/nist_csf.md`.

This document is the honest answer to the procurement question *"Is PreceptualAI FIPS 140-3 validated?"* It is intentionally precise about what is true today, what is true when the operator deploys onto a FIPS-mode host, and what would be required to claim a module-level CMVP certificate.
...
PreceptualAI has **no FIPS 140-3 module-level certificate** today. We are not on the [CMVP active-validation list](https://csrc.nist.gov/projects/cryptographic-module-validation-program/validated-modules) and have not begun a CMVP submission.

What PreceptualAI *does* offer:

1. **Transitive FIPS inheritance on a FIPS-mode host.** PreceptualAI's cryptographic primitives are entirely sourced from CPython's `ssl` module (against the host OpenSSL) and the `cryptography` package (which uses OpenSSL via `cffi`). On a FIPS-mode RHEL 9 / Ubuntu 22.04 LTS host where the OpenSSL provider is itself a CMVP-validated module (e.g. Red Hat's certificate #4746 against OpenSSL 3.0.7), PreceptualAI's TLS, hashing, and JWT signing operations execute against the validated provider. The application boundary is not validated, but the cryptography boundary is.
2. **A documented cryptographic-primitive inventory.** §2 below lists every primitive PreceptualAI uses, with `file:line` citations to the source. An auditor can verify the claim by `grep`. There are no hidden primitives.
...
| 11 | RFC 3161 timestamp signatures | Audit chain anchoring | `src/horizon_ric/evidence/rfc3161.py:47-52` (uses `rfc3161-client`) | RSA-PKCS1-v1_5 + SHA-256 inside the TSA's response. **Approved** (TSA-side); PreceptualAI only verifies. |
...
| 6 | External crypto review (1 reviewer-week, specialist) — sign-off letter on the inventory in §2 and the boundary list in §3 | external | signed PDF, attached to `marketplace/` |

This path produces **"FIPS-Inside" deployability**, not a CMVP certificate. A full PreceptualAI-as-a-cryptographic-module CMVP submission is a separate ~$300k, 18-month engagement we have explicitly chosen *not* to pursue ahead of pilot revenue. It remains on the Phase-3 roadmap.
...
It **is** the honest, line-cited inventory a procurement team can put in front of their security engineer to decide whether PreceptualAI clears the bar for *their* deployment posture today, and what closing the residual gap would cost.
