"""
Async gRPC inference server for the PreceptualAI xApp.

Implements the ``PreceptualAI`` service defined in ``proto/preceptualai.proto``.
...
from preceptualai.xapp.config import SpectralConfig
from preceptualai.xapp.inference_engine import InferenceEngine
...
try:
    ...
    _HAS_PROTO = False
    preceptualai_pb2 = None  # type: ignore[assignment]
    preceptualai_pb2_grpc = None  # type: ignore[assignment]
...
class PreceptualAIServicer:
    """
    gRPC servicer implementing the PreceptualAI proto service.
    ...
    async def Predict(self, request: object, context: grpc.aio.ServicerContext) -> object:
        ...
        return preceptualai_pb2.PredictResponse(  # type: ignore[attr-defined]
            action=action,
            action_probs=probs,
            latency_ms=latency,
    ...
    async def PredictStream(
        ...
        async for request in request_iterator:
            ...
            yield preceptualai_pb2.PredictResponse(  # type: ignore[attr-defined]
                action=action,
                action_probs=probs,
                latency_ms=latency,
                request_id=request.request_id,  # type: ignore[attr-defined]
    ...
    async def GetHealth(self, request: object, context: grpc.aio.ServicerContext) -> object:
        ...
        status = preceptualai_pb2.HealthStatus.ServingStatus.SERVING  # type: ignore[attr-defined]

        return preceptualai_pb2.HealthStatus(  # type: ignore[attr-defined]
            status=status,
    ...
    async def GetMetrics(self, request: object, context: grpc.aio.ServicerContext) -> object:
        ...
        return preceptualai_pb2.MetricsResponse(  # type: ignore[attr-defined]
...
async def serve(config: SpectralConfig) -> None:
    """
    Start the PreceptualAI gRPC server.
    ...
    servicer = PreceptualAIServicer(engine=engine, config=config)

    server = grpc.aio.server()
    preceptualai_pb2_grpc.add_PreceptualAIServicer_to_server(servicer, server)
    ...
    if config.grpc.reflection:
        try:
            ...
            service_names = (
                preceptualai_pb2.DESCRIPTOR.services_by_name["PreceptualAI"].full_name,
    ...
    await server.start()
    logger.info("PreceptualAI gRPC server listening on %s", listen_addr)
...
def run_server(config_path: Optional[str] = None) -> None:
    ...
    async event loop.
    """
    from preceptualai.monitoring.logging import configure_logging
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="PreceptualAI gRPC Inference Server")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
        help="Path to preceptualai config YAML file.",
    )
